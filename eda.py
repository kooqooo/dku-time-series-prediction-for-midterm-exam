...
from config import *
from utils.preprocess import convert_to_datetime, add_time_features, drop_columns

df = pd.read_csv("data/train.csv")

df["datetime"] = df["yymm"].apply(convert_to_datetime)
df = add_time_features(df, datetime_column="datetime")
df = drop_columns(df, columns=["yymm", "hour_sin", "hour_cos", "weekday_sin", "weekday_cos"])

# 시계열 플롯
plt.figure(figsize=(25, 5))
plt.plot(df['datetime'], df['Target'])  # 'Target'을 실제 타겟 변수 이름으로 변경하세요
...
if len(df) >= 2 * 24 * 7:  # 최소 2주 데이터
    df_daily = df.set_index('datetime').resample('D')['Target'].mean()
...
# 하루를 144개의 10분 간격으로 분할
df['time_of_day_sin'] = np.sin(df['time_of_day'] * (2 * np.pi / 144))
df['time_of_day_cos'] = np.cos(df['time_of_day'] * (2 * np.p / 144))
